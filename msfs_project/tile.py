...
class MsfsTile(MsfsSceneObject):
    ...
    def generate_height_data(self, height_map_xml, group_id, altitude, inverted=False, positioning_file_path="", water_bridge_mask_file_path="", ground_mask_file_path=""):
        if not self.lods:
            return
        ...
        if os.path.isdir(lod.folder):
            height_data, width, altitude, grid_limit = lod.calculate_height_data(self.coords[0], self.coords[2], altitude, inverted=inverted, positioning_file_path=positioning_file_path, water_bridge_mask_file_path=water_bridge_mask_file_path, ground_mask_file_path=ground_mask_file_path)
