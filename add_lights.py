...
try:
    ...
    xml = ObjectsXml(os.path.dirname(args.scene_definition_file), os.path.basename(args.scene_definition_file))

    landmarkLocation.add_lights(model_files_paths, positioning_files_paths, args.mask_file_path, float(args.lat), float(args.lon), float(args.alt), args.geocode_prefix, xml, debug=debug)
except:
    pass
